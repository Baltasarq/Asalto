   ...
   3 GOSUB 9998: PEN 12:PRINT "Asalto y castigo": PEN 3:PRINT:PRINT "baltasarq@gmail.com"
   5 PEN 7:PRINT:PRINT "C.A.A.D.": PRINT "http://www.caad.es/": PRINT:PRINT:GOSUB 9965
   7 LET Cols=80
  10 GOSUB 9998
  ...
  16 LET nloc=1: LET sig=0: LET trSax=0: LET hablado=0: LET acc=-1: LET uno=-1: LET otro=-1: LET cortado=0: LET j$="No lo veo, o no es importante.":LET chrA=ASC("A"): LET chrZ=ASC("Z"): LET diff=ABS (ASC("a")-chrA)
18 LET k$="Direcciones: n(orte), s(ur), e(ste), o(este), arriba, abajo. (M)irar redescribe un lugar, (ex)aminar permite examinar un objeto, o, al no indicar nada, a ti. Se puede cortar, nadar, atacar, empujar o golpear algo, cogerlo, dejarlo, y tirarlo."
 ...
 140 GOTO 120
 500 REM accs
 505 LET Accs=18: DIM a$(18): DIM n(18): RESTORE 525
 510 FOR i=1 TO Accs
 ...
 544 IF n>1 THEN GOTO 550
 546 IF m$="e" THEN LET acc=9: RETURN
 548 IF m$="m" THEN LET acc=13: RETURN
 550 IF n>2 THEN IF left$(m$,3)="nad" THEN LET acc=6: RETURN
 551 IF n>6 THEN IF left$(m$,6)="recort" THEN LET acc=4: RETURN
 552 IF n>4 THEN IF left$(m$,4)="afil" THEN LET acc=4: RETURN
 553 IF n>3 THEN IF left$(m$,3)="mat" THEN LET acc=4: RETURN
 554 IF m$="examinate" OR m$="examinarte" THEN LET acc=10: RETURN
 555 IF m$="fin" THEN LET acc=17: RETURN
 556 IF n>4 THEN IF left$(m$,4)="cort" THEN LET acc=4: RETURN
 557 IF n>5 THEN IF left$(m$,5)="empuj" THEN LET acc=4: RETURN
 558 IF n>5 THEN IF left$(m$,5)="golpe" THEN LET acc=4: RETURN
 559 IF n>4 THEN IF left$(m$,4)="romp" THEN LET acc=4: RETURN
 560 IF n>7 THEN LET n=7: LET m$=left$(m$,7)
 565 LET i=1: LET f=Accs+1: LET m=2
 575 LET m=INT ((i+f)/2)
 580 IF left$(a$(m),n)=m$ THEN LET acc=m: RETURN
 582 IF i=m THEN RETURN
 585 IF a$(m)>m$ THEN LET f=m: GOTO 575
 590 LET i=m: GOTO 575
1000 REM alcance
1005 LET aqui=0
1010 IF o(obj,1)=0 OR o(obj,1)=nloc THEN LET aqui=1
1015 RETURN
2000 LET t$="": REM objsLoc
2005 FOR n=1 TO Objs
2010 IF o(n,1)=nloc AND o(n,2)=2 THEN LET t$=t$+"  - "+CHR$(ASC(left$(n$(n),1))-diff)+right$(n$(n),len(n$(n))-1)+CHR$(13)+CHR$(10): GOTO 2035
2015 IF o(n,1)=nloc THEN LET t$=t$+"  - "+n$(n)+CHR$ (13)+chr$(10)
2035 NEXT n
2045 IF t$<>"" THEN PRINT "Puedes ver:"+CHR$(13)+chr$(10)+t$
2050 RETURN
2500 REM proc1
2502 LET m$=""
2505 IF nloc=11 OR nloc=38 OR nloc=44 THEN LET o(15,1)=nloc
2510 IF nloc=16 THEN LET m$="En la distancia, por entre los resquicios de las rocas, y allende el canal de agua, los sajones tratan de buscar la salida que encontraste por casualidad."
2515 IF nloc<10 AND d(8,1)<>0 THEN LET m$="Tus hombres siguen tus pasos."
2520 IF o(2,1)<>255 AND o(16,2)=0 AND (nloc=46 OR sig=1) THEN LET m$="Tu benefactor te sigue, esperanzado.": LET o(2,1)=nloc: RETURN
2530 IF nloc=28 THEN LET d(28,3)=0: RETURN
2540 IF nloc=31 AND d(31,1)=0 THEN LET m$="Las rocas bloquean el camino.": RETURN
2545 IF nloc=31 THEN LET m$="Las rocas yacen desmoronadas a lo largo del pasaje.": RETURN
2999 RETURN
3000 REM proc2
3010 IF nloc<>8 OR d(8,1)=0 THEN goto 3100
...
3100 IF trSax>0 AND nloc<12 THEN LET trSax=trSax+1: LET t$="No sabes cuanto tiempo te queda ...": GOSUB 9850: PRINT
3200 IF trSax>10 AND nloc<12 THEN LET t$="Los sajones te capturan. Su general, sonriendo ampliamente, dice: 'bien, bien'. Del gran Ulfius podremos sacar una buena ventaja.": GOSUB 9840: GOSUB 9960: let acc=17:goto 7000
3300 IF nloc<10 AND d(8,1)=0 THEN LET t$="Tus hombres luchan con denuedo contra los sajones.": PRINT : GOSUB 9850
3400 LET obj=4: GOSUB 1000: IF nloc>19 AND aqui=0 THEN LET t$="Ante la reinante e intimidante oscuridad, retrocedes a donde puedes ver.": GOSUB 9850: GOSUB 9960: LET nloc=17: GOSUB 9998: GOSUB 9980: RETURN
3500 IF nloc<>51 THEN goto 4999
...
4999 RETURN
5000 REM locs
5010 LET Locs=51: DIM h$(51): RESTORE 5027
...
5282 REM nObjs
5287 LET Objs=23: RESTORE 5312: DIM n$(23)
...
5651 DATA 255,0
7000 REM proc0
7005 IF acc=-1 THEN LET t$="No conozco ese verbo.": GOSUB 9850: RETURN
7010 IF n(acc)=1 AND uno=-1 THEN LET t$=j$: GOSUB 9850: RETURN
7015 IF n(acc)=2 AND (uno=-1 OR otro=-1) THEN LET t$=j$: GOSUB 9850: RETURN
7020 IF n(acc)>0 THEN LET obj=uno: GOSUB 1000: IF aqui=0 THEN LET t$=j$: GOSUB 9850: RETURN
7030 IF n(acc)=2 AND otro<>-1 THEN LET obj=otro: GOSUB 1000: IF aqui=0 THEN LET t$=j$: GOSUB 9850: RETURN
7045 IF acc<>17 THEN GOTO 7055
7050 IF acc=17 THEN LET m$="Volver a intentarlo (S/N):": GOSUB 9940: IF left$(m$,1)="n" OR left$(m$,1)="N" THEN gosub 9998: NEW
7052 RUN
7055 IF acc=13 THEN GOSUB 9998: GOSUB 9980: RETURN
7060 IF acc=10 AND uno=-1 THEN GOSUB 7950: RETURN
7062 IF acc=10 THEN LET obj=uno: GOSUB 1000: IF aqui=1 THEN LET t$=o$(uno): GOSUB 9850: RETURN
7064 IF acc=10 THEN LET t$=j$: GOSUB 9850: RETURN
7065 IF acc=14 THEN LET numDir=1: GOSUB 7900: RETURN
7070 IF acc=16 THEN LET numDir=2: GOSUB 7900: RETURN
7075 IF acc=9 THEN LET numDir=3: GOSUB 7900: RETURN
7080 IF acc=15 THEN LET numDir=4: GOSUB 7900: RETURN
7085 IF acc=3 THEN LET numDir=5: GOSUB 7900: RETURN
7090 IF acc=1 THEN LET numDir=6: GOSUB 7900: RETURN
7095 IF acc=5 THEN LET t$=k$: GOSUB 9850: GOSUB 9960: RETURN
7100 IF acc=7 THEN GOSUB 7515: RETURN
7110 IF acc=8 THEN GOSUB 7500: RETURN
7115 IF acc<>6 OR nloc<>11 THEN goto 7150
...
7130 PRINT : LET t$="Consigues emerger, si bien en un sitio desconocido de la caverna ...": GOSUB 9850: GOSUB 9960: GOSUB 9998: GOSUB 9980: RETURN
7150 IF acc=6 THEN LET t$="No tiene sentido nadar ahora.": GOSUB 9850: RETURN
7160 IF acc=4 THEN GOSUB 7530: RETURN
7170 IF acc=11 THEN GOSUB 7600: RETURN
7180 IF acc=18 THEN GOSUB 7700: RETURN
7190 IF acc<>2 OR nloc<>47 THEN goto 7250
7200 LET obj=16: GOSUB 1000: IF aqui<>1 THEN goto 7250
...
7230 LET o(2,1)=255: LET o(16,1)=255: LET o$(19)="Entreabierta.": LET d(47,4)=48: RETURN
7250 IF acc=2 AND nloc=47 THEN LET t$="El candado bloquea la puerta.": GOSUB 9850: RETURN
7260 IF acc=2 THEN LET t$="No hay nada que abrir ahora.": GOSUB 9850: RETURN
7270 IF acc=12 THEN GOSUB 7715: RETURN
7499 GOSUB 7990: RETURN
7500 REM deja
7502 IF uno=10 AND nloc<12 THEN LET t$="No, es lo que queda de tu padre.": GOSUB 9850: RETURN
7504 IF uno=4 THEN LET t$="No, sin luz es imposible moverse por la caverna.": GOSUB 9850: RETURN
7506 IF o(uno,1)=0 THEN LET o(uno,1)=nloc: LET t$="Hecho.": GOSUB 9850: RETURN
7510 LET t$="No llevas eso contigo.": GOSUB 9850: RETURN
7515 REM coge
7516 IF o(uno,1)=0 THEN LET t$="Pero si ya lo tienes ...": GOSUB 9850: RETURN
7518 IF uno=16 AND o(16,2)=1 THEN LET t$="Ambrosio la retiene consigo.": GOSUB 9850: RETURN
7520 IF o(uno,2)<>0 THEN LET t$="No es algo que se pueda coger.": GOSUB 9850: RETURN
7525 LET o(uno,1)=0: LET t$="Hecho.": GOSUB 9850: RETURN
7530 REM romper
7532 LET n=0: LET obj=otro: IF otro=-1 THEN LET otro=10: LET obj=10: LET n=1
7535 GOSUB 1000
7536 IF uno=22 and otro=10 and cortado=0 and n<>1 and aqui=1 THEN LET t$="Sabes que el resultado es la hoja de tu espada, mellada. No.": GOSUB 9850: RETURN
7537 IF uno=22 and otro=17 and cortado=0 and aqui=1 THEN LET cortado=1:LET t$="Con el pedernal, recortas la punta del tronco.": GOSUB 9850: RETURN
...
7540 IF uno=6 AND ((aqui=1 AND otro=10) OR n=1) THEN LET o(6,1)=255: LET o(11,1)=0: LET o(12,1)=0: LET o(23,1)=0: LET t$="Rasgas la capa, como buenamente puedes.": GOSUB 9850: RETURN
7541 IF uno=6 THEN GOTO 7595
7542 IF uno=20 AND otro=22 AND aqui=1 AND n=0 AND cortado=1 THEN LET o(20,1)=255: LET d(31,1)=32: LET t$="Haciendo palanca, consigues desencajar una, y el resto caen por su propio peso.": GOSUB 9850: RETURN
7544 IF uno=20 AND otro=22 AND aqui=1 AND n=0 THEN LET t$="Lo intentas con el tronco, pero la punta es demasiado gruesa, y no penetra entre los resquicios de las rocas.":GOSUB 9850: RETURN
7545 IF uno=20 AND otro=10 THEN LET t$="Tu espada no hace nada, ni, por descontado, tus manos desnudas.": GOSUB 9850: RETURN
7550 IF aqui=0 THEN LET t$="En cualquier caso, no con las manos desnudas.": GOSUB 9850: RETURN
7552 IF uno=20 AND otro=22 THEN LET o(20,1)=255: LET d(31,1)=32: LET t$="Haciendo palanca, consigues desencajar una, y el resto caen por su propio peso.": GOSUB 9850: RETURN
7555 LET obj=3: GOSUB 1000: IF uno<>17 or otro<>10 or aqui<>1 THEN goto 7560
7557 LET o(3,1)=255: LET o(4,1)=nloc: LET n$(3)="amorfa": LET n$(4)="antorcha": LET t$="Poderosas chispas salen del choque entre espada y pedernal, encendiendo la antorcha.": GOSUB 9850: RETURN
7560 IF uno=17 AND otro=10 THEN LET t$="Ante el potente choque con la espada, poderosas chispas saltan en todas direcciones.": GOSUB 9850: RETURN
7565 IF uno=17 THEN GOTO 7595
7570 IF uno=21 AND otro=10 THEN LET t$="Ante los amenazadores tajos, la serpiente huye.": LET d(43,2)=44: LET o(21,1)=255: GOSUB 9850: RETURN
7575 IF uno=21 THEN GOTO 7595
7590 LET t$="Eso no es lo adecuado ahora.": GOSUB 9850: RETURN
7595 LET t$="Con eso no...": GOSUB 9850: RETURN
7600 REM habla
7601 IF uno<>2 then goto 7660
7602 if o(2,1)<>19 THEN goto 7625
...
7620 LET t$="Ulfius se da la vuelta para ver si Ambrosio le sigue, pero ... ha desaparecido.": GOSUB 9850: GOSUB 9970: PRINT : LET t$="Ulfius piensa entonces en el hecho curioso de que supiera su nombre.": GOSUB 9850: GOSUB 9960: RETURN
7625 IF nloc<>46 or sig<>0 THEN goto 7655
...
7655 IF nloc>=45 AND nloc<=47 THEN LET t$="- Por favor, Ulfius, cumple tu promesa. Toma la llave en tu mano y abre la puerta de la cueva.": LET o(16,1)=0: LET o(16,2)=0: LET sig=1: GOSUB 9840: RETURN
7660 IF uno=13 THEN GOSUB 7670: RETURN
7665 LET t$="No tiene sentido hablar con eso.": GOSUB 9850: RETURN
7670 IF hablado=0 THEN LET t$="- Me llamo Ulfius y ...": GOSUB 9840: LET t$="El hombre asiente, impaciente.": GOSUB 9850: LET t$="- Somos refugiados de la gran guerra. Buscamos la paz.": GOSUB 9840
7680 LET hablado=1
7685 LET obj=18: GOSUB 1000: IF aqui<>1 THEN goto 7697
...
7690 LET t$="- La piedra debe devolverse a su lugar de encierro.": GOSUB 9840: LET t$="Un hombre te arrebata la piedra y se la lleva.": GOSUB 9850: RETURN
7697 LET obj=10: GOSUB 1000: IF aqui=1 THEN LET t$="El hombre se enfurece, y alza su mano indicando al norte.": GOSUB 9850: LET t$="- Nadie portando armas puede pasar.": GOSUB 9840: RETURN
7699 LET d(28,3)=29: LET t$="El hombre, calmado, indica hacia el este y habla:": GOSUB 9850: LET t$="- Si vienes en paz, puedes ir en paz.": GOSUB 9840: LET t$="Todos se apartan y permiten ahora el paso al este.": RETURN
7700 REM tira
7705 IF nloc=19 AND (uno=10 OR uno=18) AND hablado=1 THEN LET o(uno,1)=39: LET t$="La corriente lo arrastra fuertemente, hasta perderlo de vista.": GOSUB 9850: RETURN
7707 IF nloc=19 THEN LET t$="No quieres perder eso.": GOSUB 9850: RETURN
7709 IF nloc=13 OR nloc=16 THEN LET t$="No hay suficiente profundidad.": GOSUB 9850: RETURN
7710 LET t$="No tiene sentido tirar nada ahora.": GOSUB 9850: RETURN
7715 REM mete
7720 IF otro<>14 THEN LET t$="No tiene sentido meter nada en eso.": GOSUB 9850: RETURN
7730 IF uno=18 OR uno=9 THEN LET o(uno,1)=255
7735 IF o(18,1)=255 AND o(9,1)=255 THEN LET d(41,2)=42: LET t$="La gran roca se desplaza y deja el paso libre.": GOSUB 9850: RETURN
7737 IF uno<>18 AND uno<>9 THEN LET t$="No encaja.": GOSUB 9850: RETURN
7740 LET t$="Encaja: metido. Desaparece en su interior.": GOSUB 9850: RETURN
7900 REM movs
7905 IF d(nloc,numDir)>0 THEN LET nloc=d(nloc,numDir): GOSUB 9998: GOSUB 9980: RETURN
7910 LET t$="No es posible.": GOSUB 9850: RETURN
7950 LET t$="": REM inv
7952 FOR n=1 TO Objs
7954 IF o(n,1)=0 THEN LET t$=t$+"  - "+n$(n)+CHR$(13)+chr$(10)
7956 NEXT n
7958 IF t$<>"" THEN PRINT "Inventario:": PRINT : PRINT t$: GOTO 7970
...
8015 IF j<=l AND mid$(t$,j,1)=" " THEN LET j=j+1: GOTO 8015
8025 IF l>1 AND mid$(t$,l,1)=" " THEN LET l=l-1: GOTO 8025
8030 IF j>l THEN GOTO 8080
8035 LET chrAsc=ASC(MID$(t$,j,1))
8040 IF chrAsc>=chrA AND chrAsc<=chrZ THEN LET chrAsc=diff+chrAsc
8045 IF chrAsc<>ASC(" ") THEN LET m$=m$+CHR$(chrAsc): GOTO 8070
8050 GOSUB 8090
8055 IF otro<>-1 THEN GOTO 8085
8065 IF mid$(t$,j,1)=" " THEN LET j=j+1: GOTO 8065
8067 GOTO 8030
8070 LET j=j+1
8075 GOTO 8030
8080 GOSUB 8090
8085 RETURN
8090 IF NOT (m$<>"con" AND m$<>"de" AND m$<>"del" AND m$<>"el" AND m$<>"la" AND m$<>"los" AND m$<>"las" AND m$<>"a" AND m$<>"al" AND m$<>"por" AND m$<>"en" AND m$<>"y") THEN GOTO 8120
8095 IF acc<>-1 THEN GOTO 8105
8100 IF right$(m$,1)="r" THEN LET m$=left$(m$,LEN(m$)-1)
8101 GOSUB 540
8102 GOTO 8120
8105 IF uno=-1 THEN GOSUB 8157: LET uno=obj: GOTO 8120
8110 IF otro=-1 THEN GOSUB 8157: LET otro=obj
8120 LET m$="": RETURN
8155 REM locObj
8157 IF m$="anciano" or m$="jefe" THEN LET obj=13: RETURN
8160 LET n=LEN(m$): LET obj=-1: IF n=0 THEN RETURN
8162 IF n>9 THEN LET n=9: LET m$=left$(m$,9)
8165 IF n>2 THEN IF left$(m$,3)="ojo" THEN LET obj=14: RETURN
8167 IF n>6 THEN IF left$(m$,7)="agujero" THEN LET obj=14: RETURN
8170 LET i=1: LET f=Objs+1
8175 LET m=INT ((i+f)/2)
8180 IF left$(n$(m),n)=m$ THEN LET obj=m: RETURN
8185 IF m=i THEN RETURN
8190 IF n$(m)>m$ THEN LET f=m: GOTO 8194
8192 LET i=m
8194 GOTO 8175
9830 REM descMira
9835 PEN 3
...
9855 LET i=1: LET f=Cols: LET n=LEN(t$)+1
9857 IF Cols>n THEN PRINT t$: GOTO 9880
9860 IF i<n AND mid$(t$,i,1)=" " THEN LET i=i+1: LET f=f+1: GOTO 9860
9862 IF f>n THEN LET f=n: GOTO 9870
9865 IF f>1 AND mid$(t$,f,1)<>" " THEN LET f=f-1: GOTO 9865
9870 PRINT mid$(t$,i,f-i)
9875 IF f<n THEN LET i=f+1: LET f=f+Cols: GOTO 9860
9880 LET t$="": PEN 7: RETURN
9940 REM siNo()
9942 print m$;:INPUT " ", m$
9944 IF LEN(m$)=0 THEN GOTO 9942
9946 IF left$(m$,1)=" " THEN GOTO 9942
9948 LET m$=left$(m$,1)
9950 RETURN
9960 REM Puntos
9965 PEN 2:PRINT:PRINT "...":PRINT:PEN 7
9970 REM pausa
9972 IF INKEY$<>"" THEN GOTO 9972
9974 IF INKEY$="" THEN GOTO 9974
9976 RETURN
9980 REM mira
9982 GOSUB 2500: PRINT: LET t$=h$(nloc): GOSUB 9830: PEN 7
9984 IF m$<>"" THEN LET t$=m$: LET m$="": PRINT : GOSUB 9850
9986 PRINT : GOSUB 2000: RETURN
9995 REM input
9996 PEN 14:INPUT "> ", t$: IF t$="" THEN GOTO 9996
9997 PEN 7:RETURN
9998 CLS : RETURN
